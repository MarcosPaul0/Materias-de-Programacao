...
class CtrlTurma():       
    ...
    def consultaTurmas(self):
        self.limitConsult = LimiteConsultaDisciplinas(self)

    def resultadoConsulta(self, event):
        data = ''
        for turma in self.listaTurmas:
            if self.limitConsult.inputSearch.get() == turma.getDisciplina().getCodigo():
                data += 'Código: {}\n'.format(turma.getCodigo())
                data += 'Disciplina: {}\n'.format(turma.getDisciplina().getNome())
                data += 'Estudantes:\n'
                for estudante in turma.getEstudantes():
                    data += '   {} - {}\n'.format(estudante.getNroMatric(), estudante.getNome())
                data += '-------------------\n'
        if data:
            self.limitConsult.mostraJanela('Turma', data)
        else:
            self.limitConsult.mostraJanela('Turma', 'Turma não encontrada')
